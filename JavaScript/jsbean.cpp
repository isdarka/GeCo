...
JsBean::JsBean(GeCoBean model, QVector<GeCoBean> models)
{
    this->model = model;
    this->models = models;
...
QString JsBean::ucfirst(QString str)
{
    if (str.size() < 1) {
        return "";
    }
...
QString JsBean::lcFirst(QString str)
{
    if (str.size() < 1) {
        return "";
    }
...
void JsBean::generate()
{
    this->code.setName(this->model.getName());
    ...
    foreach (column, this->columns) {
        this->code.addProperty(this->lcFirst(this->ucfirst(column.getField())));
    }
    ...
    if(!this->model.getExtend().isEmpty())
    {
        ...
        this->code.setExtend(extra.getName());
    }
...
//    }
       ...
       Docblock docblock;
       docblock.setShortDescription(this->model.getName());
       docblock.setLongDescription("GeCo");
       docblock.addTag("author", "isdarka");
       docblock.addTag("category", "JavaScript");
...
GeCoBean JsBean::getByExntendName(QString extend)
{
    GeCoBean model;
